...
namespace ImGui
{
    ...
    IMGUI_API ImGuiWindowSettings*  FindWindowSettings(ImGuiID m_Id);
...
//#endif
    ...
    IMGUI_API bool          ItemHoverable(const ImRect& bb, ImGuiID m_Id);
    ...
    inline bool FocusableItemRegister(ImGuiWindow* window, ImGuiID m_Id)  { IM_ASSERT(0); IM_UNUSED(window); IM_UNUSED(m_Id); return false; } // -> pass ImGuiItemAddFlags_Inputable flag to ItemAdd()
    ...
    IMGUI_API bool          BeginChildEx(const char* name, ImGuiID m_Id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
    ...
    IMGUI_API bool          BeginPopupEx(ImGuiID m_Id, ImGuiWindowFlags extra_flags);
    ...
    IMGUI_API void          ActivateItem(ImGuiID m_Id);   // Remotely activate a button, checkbox, tree node etc. given its unique ID. activation is queued and processed on the next frame when the item is encountered again.
    ...
    IMGUI_API bool          BeginTableEx(const char* name, ImGuiID m_Id, int columns_count, ImGuiTableFlags flags = 0, const ImVec2& outer_size = ImVec2(0, 0), float inner_width = 0.0f);
    ...
    IMGUI_API void          RenderNavHighlight(const ImRect& bb, ImGuiID m_Id, ImGuiNavHighlightFlags flags = ImGuiNavHighlightFlags_TypeDefault); // Navigation highlight
    ...
    IMGUI_API bool          CollapseButton(ImGuiID m_Id, const ImVec2& pos);
    ...
    IMGUI_API bool          ImageButtonEx(ImGuiID m_Id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col);
    ...
    IMGUI_API bool          SplitterBehavior(const ImRect& bb, ImGuiID m_Id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend = 0.0f, float hover_visibility_delay = 0.0f);
    IMGUI_API bool          TreeNodeBehavior(ImGuiID m_Id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end = NULL);
    IMGUI_API bool          TreeNodeBehaviorIsOpen(ImGuiID m_Id, ImGuiTreeNodeFlags flags = 0);                     // Consume previous SetNextItemOpen() data, if any. May return true when logging
    ...
    inline ImGuiInputTextState* GetInputTextState(ImGuiID m_Id)   { ImGuiContext& g = *GImGui; return (g.InputTextState.ID == m_Id) ? &g.InputTextState : NULL; } // Get input text state if active
