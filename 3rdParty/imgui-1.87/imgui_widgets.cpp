...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID m_Id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    bool pressed = false;
    bool hovered = ItemHoverable(bb, m_Id);

    // Drag source doesn't report as hovered
    if (hovered && g.DragDropActive && g.DragDropPayload.SourceId == m_Id && !(g.DragDropSourceFlags & ImGuiDragDropFlags_SourceNoDisableHover))
        hovered = false;
    ...
    if (hovered && (flags & ImGuiButtonFlags_AllowItemOverlap) && (g.HoveredIdPreviousFrame != m_Id && g.HoveredIdPreviousFrame != 0))
        hovered = false;
    ...
    if (hovered)
    {
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if (g.ActiveId == m_Id && (flags & ImGuiButtonFlags_Repeat))
    ...
    if (g.NavId == m_Id && !g.NavDisableHighlight && g.NavDisableMouseHover && (g.ActiveId == 0 || g.ActiveId == m_Id || g.ActiveId == window->MoveId))
...
bool ImGui::CollapseButton(ImGuiID m_Id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ScrollbarEx(bb, m_Id, axis, &scroll, (ImS64)size_avail, (ImS64)size_contents, rounding_corners);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID m_Id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
...
bool ImGui::ImageButtonEx(ImGuiID m_Id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    return ImageButtonEx(m_Id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col);
}
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID m_Id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    ButtonBehavior(bb_interact, m_Id, &hovered, &held, ImGuiButtonFlags_FlattenChildren | ImGuiButtonFlags_AllowItemOverlap);
    if (hovered)
...
bool ImGui::DragBehavior(ImGuiID m_Id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == m_Id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0])
            ClearActiveID();
        else if (g.ActiveIdSource == ImGuiInputSource_Nav && g.NavActivatePressedId == m_Id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != m_Id)
        return false;
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool hovered = ItemHoverable(frame_bb, m_Id);
    ...
    if (!temp_input_is_active)
    {
        ...
        if (input_requested_by_tabbing || clicked || double_clicked || g.NavActivateId == m_Id || g.NavActivateInputId == m_Id)
        {
            SetActiveID(m_Id, window);
            SetFocusID(m_Id, window);
            FocusWindow(window);
            ...
            if (temp_input_allowed)
                if (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || double_clicked || g.NavActivateInputId == m_Id)
        ...
        if (g.IO.ConfigDragClickToInputText && temp_input_allowed && !temp_input_is_active)
            ...
            {
                g.NavActivateId = g.NavActivateInputId = m_Id;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        const bool input_requested_by_tabbing = temp_input_allowed && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
        const bool clicked = (hovered && g.IO.MouseClicked[0]);
        if (input_requested_by_tabbing || clicked || g.NavActivateId == m_Id || g.NavActivateInputId == m_Id)
        {
            SetActiveID(m_Id, window);
            SetFocusID(m_Id, window);
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || g.NavActivateInputId == m_Id))
                temp_input_is_active = true;
        }
    }
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool hovered = ItemHoverable(frame_bb, m_Id);
    if ((hovered && g.IO.MouseClicked[0]) || g.NavActivateId == m_Id || g.NavActivateInputId == m_Id)
    {
        SetActiveID(m_Id, window);
        SetFocusID(m_Id, window);
        FocusWindow(window);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool input_requested_by_nav = (g.ActiveId != m_Id) && ((g.NavActivateInputId == m_Id) || (g.NavActivateId == m_Id && g.NavInputSource == ImGuiInputSource_Keyboard));
    ...
    if ((init_state && g.ActiveId != m_Id) || init_changed_specs)
    {
        ...
        const bool recycle_state = (state->ID == m_Id && !init_changed_specs);
        if (recycle_state)
        {
    ...
    const bool render_cursor = (g.ActiveId == m_Id) || (state && user_scroll_active);
    ...
    if (clear_active_id && g.ActiveId == m_Id)
        ClearActiveID();
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID m_Id, ImGuiTreeNodeFlags flags)
{
...
bool ImGui::TreeNodeBehavior(ImGuiID m_Id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    bool is_open = TreeNodeBehaviorIsOpen(m_Id, flags);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
    {
        ...
        {
            SetNavID(m_Id, window->DC.NavLayerCurrent, window->DC.NavFocusScopeIdCurrent, WindowRectAbsToRel(window, bb)); // (bb == NavRect)
            g.NavDisableHighlight = true;
        }
    }
    if (pressed)
    ...
    RenderNavHighlight(bb, m_Id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(m_Id, frame_bb.GetSize());
    return true;
}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    {
        if (menu_is_open)
            menu_is_open = BeginPopupEx(m_Id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
        else
            g.NextWindowData.ClearFlags();          // we behave like Begin() and need to consume those values
        return menu_is_open;
    }
    ...
    const bool hovered = (g.HoveredId == m_Id) && enabled;
    if (menuset_is_open)
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (menu_is_open && !hovered && g.HoveredWindow == window && g.HoveredIdPreviousFrame != 0 && g.HoveredIdPreviousFrame != m_Id && !moving_toward_other_child_menu)
            want_close = true;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID m_Id = TabBarCalcTabID(tab_bar, label);
    ...
    if (p_open && !*p_open)
    {
        ItemAdd(ImRect(), m_Id, NULL, ImGuiItemFlags_NoNav | ImGuiItemFlags_NoNavDefaultFocus);
        return false;
    }
    ...
    if (tab_appearing && (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) && tab_bar->NextSelectedTabId == 0)
        if (!tab_bar_appearing || tab_bar->SelectedTabId == 0)
            if (!is_tab_button)
                tab_bar->NextSelectedTabId = m_Id;  // New tabs gets activated
    if ((flags & ImGuiTabItemFlags_SetSelected) && (tab_bar->SelectedTabId != m_Id)) // SetSelected can only be passed on explicit tab bar
        if (!is_tab_button)
            tab_bar->NextSelectedTabId = m_Id;
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed("#CLOSE", NULL, m_Id) : 0;
    bool just_closed;
    bool text_clipped;
    TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding, label, m_Id, close_button_id, tab_contents_visible, &just_closed, &text_clipped);
    ...
    if (text_clipped && g.HoveredId == m_Id && !held && g.HoveredIdNotActiveTimer > g.TooltipSlowDelay && IsItemHovered())
