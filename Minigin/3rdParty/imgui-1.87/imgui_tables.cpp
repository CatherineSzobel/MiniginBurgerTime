...
bool    ImGui::BeginTableEx(const char* name, ImGuiID m_Id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ImGuiContext& g = *GImGui;
    ...
    const ImGuiID instance_id = m_Id + instance_no;
...
{
    IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
    ImGuiID m_Id = table->ID + 1 + (instance_no * table->ColumnsCount) + column_n;
    return m_Id;
}
...
void ImGui::TableHeader(const char* label)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, m_Id, &hovered, &held, ImGuiButtonFlags_AllowItemOverlap);
    if (g.ActiveId != m_Id)
        SetItemAllowOverlap();
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* name)
{
    ImGuiID m_Id = 0;
    int columns_count = 0;
    if (sscanf(name, "0x%08X,%d", &m_Id, &columns_count) < 2)
        return NULL;
...
ImGuiOldColumns* ImGui::FindOrCreateColumns(ImGuiWindow* window, ImGuiID m_Id)
{
    ...
    for (int n = 0; n < window->ColumnsStorage.Size; n++)
        if (window->ColumnsStorage[n].ID == m_Id)
